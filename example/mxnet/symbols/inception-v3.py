...
def Inception7B(data,
    ...
    return mx.sym.Concat(
        *[tower_3x3, tower_d3x3, pooling], name=f'ch_concat_{name}_chconcat'
    )
...
def Inception7C(data,
                ...
                name):
    tower_1x1 = Conv(
        data=data, num_filter=num_1x1, kernel=(1, 1), name=f'{name}_conv'
    )
    tower_d7 = Conv(
        data=data, num_filter=num_d7_red, name=f'{name}_tower', suffix='_conv'
    )
    tower_d7 = Conv(
        data=tower_d7,
        num_filter=num_d7_1,
...
def Inception7D(data,
                ...
                name):
    tower_3x3 = Conv(
        data=data, num_filter=num_3x3_red, name=f'{name}_tower', suffix='_conv'
    )
    tower_3x3 = Conv(
        data=tower_3x3,
        num_filter=num_3x3,
    ...
    return mx.sym.Concat(
        *[tower_3x3, tower_d7_3x3, pooling], name=f'ch_concat_{name}_chconcat'
    )
...
def Inception7E(data,
    ...
    tower_d3 = Conv(
        data=data, num_filter=num_d3_red, name=f'{name}_tower', suffix='_conv'
    )
    tower_d3_a = Conv(
        data=tower_d3,
        num_filter=num_d3_1,
