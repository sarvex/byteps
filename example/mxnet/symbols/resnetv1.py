...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False):
    ...
    if bottle_neck:
        ...
        act1 = mx.sym.Activation(data=bn1, act_type='relu', name=f'{name}_relu1')
        ...
        act2 = mx.sym.Activation(data=bn2, act_type='relu', name=f'{name}_relu2')
        ...
        return mx.sym.Activation(
            data=bn3 + shortcut, act_type='relu', name=f'{name}_relu3'
        )
    else:
        conv1 = mx.sym.Convolution(
            data=data,
        ...
        if memonger:
            shortcut._set_attr(mirror_stage='True')
        return mx.sym.Activation(
            data=bn2 + shortcut, act_type='relu', name=f'{name}_relu3'
        )
...
def get_symbol(num_classes, num_layers, image_shape, conv_workspace=256, dtype='float32', **kwargs):
    ...
    if height <= 28:
        ...
        if (num_layers-2) % 9 == 0 and num_layers >= 164:
            ...
            raise ValueError(
                f"no experiments done on num_layers {num_layers}, you can do it yourself"
            )
